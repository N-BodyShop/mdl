...
/*
 */
int mdlInitialize(MDL *pmdl,char **argv,void (*fcnChild)())
{
	MDL mdl,tmdl;
	...
	*pmdl = mdl;
	if (nThreads > 1) {
		...
		for (i=0;i<nThreads;++i) {
             ...
             */
            tmdl->pfcnService[0] = _srvNull;
			/*
			 ** Initialize the mailbox.
			 */
			pthread_mutex_init(&tmdl->mbxOwn.mux,pthread_mutexattr_default);
			tmdl->mbxOwn.bReq = 0;
			tmdl->mbxOwn.bRpl = 0;
			tmdl->mbxOwn.bRel = 1;
			/*
			 ** Initialize the swapbox.
			 */
			pthread_mutex_init(&tmdl->swxOwn.mux,pthread_mutexattr_default);
			tmdl->swxOwn.bSet = 0;
			}
		for (i=1;i<nThreads;++i) {
			/*
...
void mdlFinish(MDL mdl)
{
	...
	for (i=0;i<mdl->nThreads;++i) {
		tmdl = mdl->pmdl[i];
		pthread_mutex_destroy(&tmdl->mbxOwn.mux);
		pthread_mutex_destroy(&tmdl->swxOwn.mux);
		/*
			...
			int *pnSndBytes,int *pnRcvBytes)
{
	...
	*pnSndBytes = 0;
	...
	while (nOutBytes && nInBytes) {
        ...
        pout->bSet = 1;
        for (i=0;i<nOutMax;++i) pout->achBuf[i] = pszOut[i];
        pthread_mutex_unlock(&pout->mux);
        while (1) {
            pthread_mutex_lock(&pin->mux);
            if (pin->bSet) break;
        ...
        pin->bSet = 0;
		for (i=0;i<nInMax;++i) pszIn[i] = pin->achBuf[i];
        pthread_mutex_unlock(&pin->mux);
		/*
...
void mdlReqService(MDL mdl,int id,int sid,char *pszIn,int nInBytes)
{
    ...
    for (i=0;i<nInBytes;++i) pmbx->msgBuf[i] = pszIn[i];
    pthread_mutex_unlock(&pmbx->mux);
	}
...
void mdlGetReply(MDL mdl,int id,char *pszOut,int *pnOutBytes)
{
    ...
    for (i=0;i<pmbx->nBytes;++i) pszOut[i] = pmbx->msgBuf[i];
    pthread_mutex_unlock(&pmbx->mux);
	}
...
void mdlHandler(MDL mdl)
{
	...
	while (sid != SRV_STOP) {
        ...
        sid = pmbx->sid;
        for (i=0;i<nInBytes;++i) mdl->msgBuf[i] = pmbx->msgBuf[i];
		pmbx->nBytes = 0;
		(*mdl->pfcnService[sid])(mdl->pp1[sid],
                                       mdl->msgBuf,nInBytes,
                                       pmbx->msgBuf,&pmbx->nBytes);
        pthread_mutex_unlock(&pmbx->mux);
		}
	}
