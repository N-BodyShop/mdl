/*
 ** MPI/shmem version of MDL for Quadrics Elan hardware.
 **
 ** This version uses the LRU cache line replacement scheme, rather than 
 ** the random scheme of the T3E.  Given the performance increase of the 
 ** processors on the new alpha platforms like LeMieux.psc.edu, the LRU 
 ** cache now seems to be more efficient.
 */
	...
	*pmdl = NULL;
	...
	while (argv[i]) {
		...
		if (!strcmp(argv[i],"+d") && !bDiag) {
			...
			fprintf(stderr,"MDL_DIAGNOSTIC: %s\n",p);
			if (!p) p = getenv("HOME");
			if (!p) sprintf(ach,"/tmp/gardner");
...
void *mdlMalloc(MDL mdl,size_t iSize)
{
    ...
    MPI_Barrier(MPI_COMM_WORLD);
    shmax = (long) iSize;
    /*printf("id %d: calling shmem_fcollect iSize=%d  shmax=%ld\n",mdl->idSelf,iSize,shmax);*/
    shmem_fcollect(shmem_array,&shmax,1,0,0,mdl->nThreads,pSync);
    shmax = 0;
    for (i=0;i<mdl->nThreads;++i) {
      /*if (mdl->idSelf==0) fprintf(stderr,"i: %d  shmem_array: %d\n",i,shmem_array[i]);*/
        if (shmax < shmem_array[i]) shmax=shmem_array[i];
    }
    MPI_Barrier(MPI_COMM_WORLD);
    
    huh = malloc(shmax);
    /*fprintf(stderr,"id %d: iSize: %d  shmax: %ld  huh: %ld\n",mdl->idSelf,iSize,shmax,huh);*/
...
void mdlFinishCache(MDL mdl,int cid)
{
	...
	if (c->iType == MDL_COCACHE) {
		...
		sprintf(ach,"Flushed %d out of %d cache lines\n",
			nFlush,c->nLines);
		mdlDiag(mdl,ach);
		}
	/*
...
void *doMiss(MDL mdl, int cid, int iIndex, int id, int iKey, int lock)
{
	...
	*/
	...
	if (iKeyVic >= 0) {
	     ...
	     */
	     ...
	     while (*pi != iVictim) {
		  if (*pi<=0) {
		       mdlprintf(mdl,"CACHE FAILURE! iVictim: %d  iKeyVic: %d  iElt: %d  iLine: %d\n",
				 iVictim,iKeyVic,iElt,iLine);
		       pi = &c->pTrans[iKeyVic & c->iTransMask];
		       while (1) {
			    mdlassert(mdl,*pi > 0);
			    mdlprintf(mdl,"     pi: %d\n",*pi);
	...
	/*if ((mdl->idSelf==1 && iVictim==916) || mdl->idSelf == 7 && iVictim==793) 
	 *     mdlprintf(mdl,"MDL: iVictim=%d assigned key %d.  iLine: %d  iElt: %d\n",iVictim,iKey,iLine,iElt);*/
	...
	*pi = iVictim;
	...
	/* For T3E with shmallocs
	 *shmem_get((long *)pLine,(long *)(c->pData + iLine*c->iLineSize),
	 *		  iLineSize_64,id);*/
	/* Here, we have to keep track of the start of pData on each PE */
	shmem_get((long *)pLine,(long *)(c->shmem_pData[id] + iLine*c->iLineSize),
			  iLineSize_64,id);
